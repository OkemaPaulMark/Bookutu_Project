...
export default function CompanyLayout() {
  ...
  const title = location.pathname.replace('/company/', '').replace('/company', 'Dashboard')
    ...
    <div className="min-h-screen bg-slate-50">
      <aside className="sidebar-width fixed inset-y-0 left-0 border-r border-slate-200 bg-white shadow-sm">
        <div className="flex items-center border-b border-slate-200 p-5">
          <div className="rounded bg-blue-600 p-2 text-white"><Bus size={14} /></div>
          <h1 className="menu-text ml-3 text-xl font-bold">Bookutu</h1>
        </div>
        ...
        <header className="border-b border-slate-200 bg-white">
          <div className="flex items-center justify-between p-4">
            ...
            <div className="hidden items-center gap-4 md:flex">
              <button className="rounded-full p-2 text-slate-600 hover:bg-slate-100" type="button">
                <Bell size={18} />
              </button>
              <div className="relative">
                <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                <input className="w-56 rounded-md border border-slate-300 py-2 pl-9 pr-3 text-sm" placeholder="Search..." type="text" />
              </div>
