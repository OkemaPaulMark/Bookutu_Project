...
class Booking(models.Model):
    ...
    company = models.ForeignKey(
        "companies.Company", on_delete=models.CASCADE, related_name="bookings"
    )
    ...
    seat = models.ForeignKey(
        "companies.BusSeat", on_delete=models.CASCADE, related_name="bookings"
    )
    ...
    source = models.CharField(
        max_length=20, choices=BOOKING_SOURCE_CHOICES, default="MOBILE_APP"
    )
    ...
    booked_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_bookings"
    )  # Staff member who created direct booking
    ...
    def confirm_booking(self):
        ...
        self.trip.save()
        logger.info(
            f"Updated trip {self.trip} booked seats: {old_booked} -> {self.trip.booked_seats}"
        )
    ...
    def cancel_booking(self, reason=""):
        ...
        BookingCancellation.objects.create(
            booking=self, reason=reason, cancelled_by=self.passenger
        )
    ...
    def calculate_cancellation_fee(self):
        ...
        departure_dt = timezone.make_aware(
            timezone.datetime.combine(
                self.trip.departure_date, self.trip.departure_time
            )
        )
        now = timezone.now()
        ...
        if hours_until_departure >= company_settings.cancellation_hours:
            ...
            return self.total_amount * (
                company_settings.cancellation_fee_percentage / 100
            )
...
class SeatReservation(models.Model):
    ...
    trip = models.ForeignKey(
        "trips.Trip", on_delete=models.CASCADE, related_name="seat_reservations"
    )
    seat = models.ForeignKey(
        "companies.BusSeat", on_delete=models.CASCADE, related_name="reservations"
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="seat_reservations"
    )
